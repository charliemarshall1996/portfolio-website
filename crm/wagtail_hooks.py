...
from wagtail.snippets.views.snippets import SnippetViewSet, SnippetViewSetGroup, hooks
...
class DealViewSet(SnippetViewSet):
    ...
    menu_label = "Deals"
    icon = "list-ol"
    list_display = ("name", "company", "stage", "amount", "expected_close_date")
...
class PricingOptionViewSet(SnippetViewSet):
    ...
    icon = "dollar"
    list_display = ("name", "service_type", "pricing_model", "base_rate", "active")
...
def register_document_urls():
    return [
        path(
            "crm/generate-contract/<int:contract_id>/",
            generate_contract_view,
            name="generate_contract",
        ),
        path(
            "crm/generate-invoice/<int:invoice_id>/",
            generate_invoice_view,
            name="generate_invoice",
        ),
        path(
            "crm/service-invoice/<int:service_id>/",
            create_service_invoice_view,
...
def generate_contract_view(request, contract_id):
    ...
    messages.success(request, f"Contract {contract.contract_id} generated successfully")
    return HttpResponseRedirect(
        reverse("wagtailadmin_snippets:edit", args=["crm", "contract", contract.id])
    )
...
def generate_invoice_view(request, invoice_id):
    ...
    return HttpResponseRedirect(
        reverse("wagtailadmin_snippets:edit", args=["crm", "invoice", invoice.id])
    )
...
def create_service_invoice_view(request, service_id):
    ...
    try:
        ...
        invoice.generate_document()
        messages.success(request, f"Invoice created for service {service.name}")
        return HttpResponseRedirect(
            reverse("wagtailadmin_snippets:edit", args=["crm", "invoice", invoice.id])
        )
        ...
        return HttpResponseRedirect(
            reverse("wagtailadmin_snippets:edit", args=["crm", "service", service.id])
        )
